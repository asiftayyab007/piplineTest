({
    doInit: function(component,event,helper){
		...
		var coordinatorObj = component.get("v.workForceRecord");
		coordinatorObj["ET_Coordinator_Line__c"] = component.get("v.lineNumber");
		component.set("v.workForceRecord",coordinatorObj);
            ...
            if(existingData != undefined ){
				...
				if(existingData.ET_Special_Requirements__c == 'Yes'){
					...
					if(existingData.Specific_Workforce_Requirements__r){
						component.set("v.existingCoordinatorSpecialData", existingData.Specific_Workforce_Requirements__r[0]);
					}
                    
                    if(existingData.Other_Cost_Requests__r){
                        component.set("v.existingCoordinatorOtherCosts", existingData.Other_Cost_Requests__r);
                    }
	...
	doRefresh: function(component,event,helper){
        ...
        if(childCmpId){
			if(childCmpId == 'coordinatorSpecialRequirements'){
				...
				cmp.prePopulateCommonWorkforceData(component.get("v.existingCoordinatorSpecialData"));
                if(component.get("v.existingCoordinatorOtherCosts")){
                    for(var otherCost of component.get("v.existingCoordinatorOtherCosts")){
    ...
    handleAddOtherCostMore: function(component,event,helper){
          if(helper.validateOtherCostFieldsPerTab(component,event,helper) ||  component.find("coordinatorOtherCostCmp") == undefined){
    ...
    loadSpecialRequirementDetails : function(component,event,helper){
        ...
        if(record && record.ET_Coordinator_Category__c){
            var masterData = component.get("v.coordinatorMasterDataMap");
            if(masterData && masterData[record.ET_Coordinator_Category__c]){
                var salaryMap = masterData[record.ET_Coordinator_Category__c];
                component.set("v.coordinatorSalaryDetailMap", salaryMap);
