({
    doinit: function (component, event, helper) {
        ...
        //alert(component.get('v.selectedEmirate'))
    },
    openModel: function (component, event, helper) {
		helper.getAmanLookups(component, event, helper);
        component.set("v.isOpen", true);
        component.set("v.isFilesNotUploaded",false);
         if(component.get("v.selectedEmirate")=='Sharjah'){
            component.set("v.VehicleInfo.Registration_Type__c",'Registered');
            component.set("v.selectedValue", 'Chassis No');
        }
    }, 
    openBulkModel: function (component, event, helper) {
		component.set("v.isBulkModel", true);
    }, 
    closeModel: function (component, event, helper) {
        ...
        //component.set("v.VehicleInfo.Vehicle_Type__c", ''); 
   ...
   clearvalues: function (component, event, helper) {
    ...
    saveVeh: function (component, event, helper) {
        ...
        // ADFCA Type is not required for Sharjah location...
        if(component.get("v.selectedEmirate") != 'Sharjah'){
        ...
        if(component.get("v.selectedValue")=='Plate Combination' || component.get("v.isOpenwhennoservice")){
            ...
            if (component.find('ETSPlate').get('v.value') == '' || component.find('ETSPlate').get('v.value') == null || component.find('ETSPlate').get('v.value') == undefined) {
                component.find('ETSPlate').setCustomValidity(component.get("v.Field_is_required"));
            ...
            {
                component.find('ETSPlate').setCustomValidity(component.get("v.Enter_only_numbers"));
                component.find('ETSPlate').reportValidity();
                isValueMissing = true;
            }else if(component.find('ETSPlate').get('v.value').length > 5){
                component.find('ETSPlate').setCustomValidity(component.get("v.Plate_No_should_be_5_numbers"));
                component.find('ETSPlate').reportValidity();
                isDataMissing = true;
            }else{
                component.find('ETSPlate').setCustomValidity("");
        ...
        if (component.get("v.VehicleInfo.Registration_Type__c")=='Un-Registered'){
            ...
            if( modelYear== ''  || modelYear == null ||  modelYear == undefined){
                component.find('ModelYear').setCustomValidity(component.get("v.Field_is_required"));
    ...
    handleFilesChange: function (component, event, helper) {
        ...
        //files = component.get("v.fileToBeUploaded");
   ...
   showButtonsEvent : function(component, event) { 
        //Get the event message attribute
        var isShowButtons = event.getParam("isShowButtons"); 
        console.log('isShowButtons page>> '+isShowButtons);
        //Set the handler attributes based on event data 
        if(!isShowButtons){
            var booking = event.getParam("booking"); 
            component.set('v.booking', booking);
            console.log('Booking page>> ', JSON.stringify(booking));
            var selectedEmirate = event.getParam("selectedEmirate");
            component.set('v.selectedEmirate', selectedEmirate.replace(/\s/g,''))
            component.set("v.isOpenRequestForService", true);
        }
        component.set("v.isShowButtons", isShowButtons);         
    },
    requestForService: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (component.isValid() && state === "SUCCESS") {
                ...
                console.log('Booking  ', response.getReturnValue());
                var selectedEmirate = component.get('v.selectedEmirate');
                component.set('v.selectedEmirate', selectedEmirate.replace(/\s/g,''))
                component.set("v.isOpenRequestForService", true);
                component.set("v.isShowButtons", false);
                var appEvent = $A.get("e.c:ETI_B2B_ShowDetails"); 
                //Set event attribute value
                appEvent.setParams({"isShowDetails" : false}); 
                appEvent.fire(); 
            }
        });
        $A.enqueueAction(action);
