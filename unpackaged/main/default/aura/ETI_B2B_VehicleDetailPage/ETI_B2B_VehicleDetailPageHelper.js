({
    ...
    getAmanLookups: function (component, event, helper) {
        ...
        if (state === "SUCCESS") {
            ...
            console.log('AmanLookups result'+JSON.stringify(result));
            var pType = [];
            var pSource = [];
            var pColor = [];
            var vType =[];
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                if(result == 'Sharjah'){
                    ...
                    component.set("v.VehicleInfo.ET_Vehicle_type__c", 'Vehicle');
                     component.set("v.selectedValue", 'Chassis No');//added by ashwini on 9-03-2023 
                }
            }else {
                var msg=component.get("v.Problem_Getting_Location");
 				var utility = component.find("ETI_UtilityMethods");
				var promise = utility.showToast(component.get("v.Info"),msg,"","dismissible","info");
            }
        });
    ...
    getRegistrationType: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                var result = response.getReturnValue();
                //For Sharjah location RegistrationType = 'Registered' only
                //adding some delay - to make it asynchronous action to get the Value of 'selectedEmirate', which getting from server in same thread...
                ...
                window.setTimeout(
                    $A.getCallback(function() {
                        ...
                        if(component.get("v.selectedEmirate") == 'Sharjah'){
                            var registrationTypesForSharjah = [{"label":"Registered","value":"Registered"}];
                            component.set("v.RegistrationType", registrationTypesForSharjah);
                        }
                        else{
    ...
    getVehicleServicType: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                var result = response.getReturnValue();
                //For Sharjah location vehicleTypes = 'Vehicle' only
                var delayInMilliseconds = 1000; //1 seconds
                window.setTimeout(
                    $A.getCallback(function() {
                        console.log('selectedEmirate = ' + component.get("v.selectedEmirate") );
                        if(component.get("v.selectedEmirate") == 'Sharjah'){
                            var vehicleTypesForSharjah = [{"label":"Vehicle","value":"Vehicle"}];
                            component.set("v.vehicleTypes", vehicleTypesForSharjah);
                        }
                        else{
             ...
             if (component.isValid() && state === "SUCCESS") {
                 if (result === 'No Service') {
                     ...
                     if(lang==null)
                         lang='en';
                     urlEvent.setParams({
                         "url": "/Business/s/b2bvehicle-pageinspection?recordId="+result+"&lang="+lang
                     });
                     urlEvent.fire();
                 }
             }
