({
        ...
        if(allowDML){
            if(step_error==null || step_error==''){
                if (flag && WearRequiredPPT) {
                    ...
                    //console.log(workStationSchedule.Id);
                    line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Work_Schedule__c=component.get("v.workSheduleInfo.Id");
                    line.relatedJobWrapper.validJobCardList[0].Filling_Technician__c= $A.get("$SObjectType.CurrentUser.Id");
    ...
    goToStep4 : function(component, event, helper) {
        ...
        if(line.jobLineItem.ETT_Initial_Activity_State__c =='Not Good' && line.jobLineItem.ETT_Final_Activity_State__c !='Good'){
            ...
            var Nextlineitem=component.get("v.Nextlineitem");
            Nextlineitem.Id=null;
            Nextlineitem.ETT_Activity_Performed__c='Filling machine temperature check';
            Nextlineitem.ETT_Type__c='Pre-Requisite Work';
            Nextlineitem.ETT_Status__c='Not Started';
    ...
    startTempratureWorks : function(component, event, helper) {
        var line=component.get("v.current_line");
        line.jobLineItem.ETT_Status__c='In-Progress';
    ...
    goToStep5 : function(component, event, helper) {
        ...
        if(line.jobLineItem.ETT_Initial_Activity_State__c =='No' && line.jobLineItem.ETT_Final_Activity_State__c !='Yes'){
			 component.set("v.step_error","Please ensure final temprature to be at 80 degree to proceed.");
        }
        else{
            line.jobLineItem.ETT_Status__c='Completed';
            line.workTracker.ETT_End_time__c=$A.localizationService.formatDateTime(new Date(),'YYYY-MM-DDThh:mm:ss.SSSZ');
    ...
    goToStep6 : function(component, event, helper) {
        ...
        var Nextlineitem=component.get("v.Nextlineitem");
        Nextlineitem.Id=null;
        Nextlineitem.ETT_Activity_Performed__c='Nail hole missing/Crack on Sidewall Check';
        Nextlineitem.ETT_Type__c='Pre-Requisite Work';
        Nextlineitem.ETT_Status__c='Not Started';
    ...
    goToStep7 : function(component, event, helper) {
        ...
        var Nextlineitem=component.get("v.Nextlineitem");
        Nextlineitem.Id=null;
        Nextlineitem.ETT_Activity_Performed__c='Tyre Cementing Check';
        Nextlineitem.ETT_Type__c='Pre-Requisite Work';
        Nextlineitem.ETT_Status__c='Not Started';
    ...
    goToStep8 : function(component, event, helper) {
        ...
        var Nextlineitem=component.get("v.Nextlineitem");
        Nextlineitem.Id=null;
        Nextlineitem.ETT_Activity_Performed__c='Tyre Cementing Dryness Check';
        Nextlineitem.ETT_Type__c='Pre-Requisite Work';
        Nextlineitem.ETT_Status__c='Not Started';
    ...
    /*goToStep10 : function(component, event, helper) {
        ...
        if(startCheckbox!=true){
            ...
            var Nextlineitem=component.get("v.Nextlineitem");
            Nextlineitem.Id=null;
            Nextlineitem.ETT_Activity_Performed__c='Filling Sholders Strip Availability Check';
    ...
    goToStep10: function(component,event,helper){
        ...
        if($A.util.isUndefinedOrNull(stepError) || stepError==""){
            ...
            var Nextlineitem=component.get("v.Nextlineitem");
            Nextlineitem.Id=null;
            Nextlineitem.ETT_Activity_Performed__c='Filling Work Check on Skived Areas/GSO/Plate';
            Nextlineitem.ETT_Type__c='Pre-Requisite Work';
            Nextlineitem.ETT_Status__c='Not Started';
    ...
    goToStep11: function(component,event,helper){
        ...
        var action = component.get('c.getJobCardLineInfoCommon');
        let skivedProdPlate = false;
        action.setParams({
            recordId : component.get("v.recordId"),
            ActivityPerformed:'Terms Read',
            sequence:4
        });
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {  
                ...
                skivedProdPlate = data.ETT_Skived_Production_Plate_date_area__c;
       ...
       component.set("v.step_error","");
        ...
        /*if((!line.relatedJobWrapper.validJobCardList[0].ETT_Inspection_Card__r.ETT_Collection_Card__r.Skiving_on_GSO_sticker_area__c && (!line.jobLineItem.ETT_Filling_Check_Production_plate_area__c))
           ||
           !line.jobLineItem.ETT_Filling_check_on_Skived_areas__c)*/
        if( !line.jobLineItem.ETT_Filling_check_on_Skived_areas__c)
        {
            component.set("v.step_error","Please do check Skiving Area");
        }else if(skivedProdPlate && !line.jobLineItem.ETT_Filling_check_on_GSO_Stricker_Area__c || !line.jobLineItem.ETT_Filling_Check_Production_plate_area__c){
             component.set("v.step_error","Please do check GSO Sticker Area/Production Plate Date Area.");
        }
        else{
              line.jobLineItem.ETT_Status__c='Completed';
        	   line.workTracker.ETT_End_time__c=$A.localizationService.formatDateTime(new Date(),'YYYY-MM-DDThh:mm:ss.SSSZ');
    ...
    goToStep13: function(component,event,helper){
        ...
        if($A.util.isUndefinedOrNull(line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Status__c) || line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Status__c==""){ 
            component.set("v.step_error","Status is mandatory to end.");
        }
        else if(line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Status__c=='Accepted' && ($A.util.isUndefinedOrNull(selectedNextStation) || selectedNextStation=="")){ 
            component.set("v.step_error","Next stage should be selected when accepting.");
        }else if(line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Status__c=='Initial Rejection' && $A.util.isUndefinedOrNull(line.jobLineItem.ETT_Remarks__c)){ 
            component.set("v.step_error","Please enter rejection comments.");
        }else{
            if(line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Status__c=='Initial Rejection'){
                component.set("v.selectedNextStation","");
            }
            line.jobLineItem.ETT_Status__c='Completed';
            line.workTracker.ETT_End_time__c=$A.localizationService.formatDateTime(new Date(),'YYYY-MM-DDThh:mm:ss.SSSZ');
            
            if($A.util.isUndefinedOrNull(line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Technician_Rejection_Remarks__c)){
                line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Technician_Rejection_Remarks__c='';
            }
            if($A.util.isUndefinedOrNull(line.jobLineItem.ETT_Remarks__c)){
                line.jobLineItem.ETT_Remarks__c='';
            }            
            line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Technician_Rejection_Remarks__c='Version:'+line.relatedJobWrapper.validJobCardList[0].ETT_No_of_Filling_Revisions__c+' :'+line.jobLineItem.ETT_Remarks__c+'\n'+line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Technician_Rejection_Remarks__c;//append the technician remarks
    ...
    confirmOnHold: function(component,event,helper){
        ...
        if(lineItemForHoldWrapper.jobLineItem.ETT_Pause_Reason__c==null || lineItemForHoldWrapper.jobLineItem.ETT_Pause_Reason__c==""){
            ...
            lineItemForHoldWrapper.jobLineItem.ETT_Revision_Number__c=line.relatedJobWrapper.validJobCardList[0].ETT_No_of_Filling_Revisions__c;
    ...
    reportDefectsAndEnd: function(component,event,helper){
        ...
        if($A.util.isUndefinedOrNull(selectedNextStation) || selectedNextStation==null || selectedNextStation=="" || selectedDefects.length==0){
            ...
            line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Technician_Rejection_Remarks__c='Version(Defect found):'+line.relatedJobWrapper.validJobCardList[0].ETT_No_of_Filling_Revisions__c+' :'+line.jobLineItem.ETT_Remarks__c+'\n'+line.relatedJobWrapper.validJobCardList[0].ETT_Filling_Technician_Rejection_Remarks__c;//append the technician remarks
