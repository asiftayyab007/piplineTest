    ...
    getJobDetails : function(component, event, helper,Nextlineitem,nextWorkTracker) {
        ...
        action.setCallback(this, function(response) {
            if (response.getState() == "SUCCESS") { 
                ...
                console.log('respWrapper.relatedJobWrapper.validJobCardList[0]------>'+JSON.stringify(respWrapper.relatedJobWrapper.validJobCardList[0]));
                
                component.set("v.cementing_end_time",respWrapper.relatedJobWrapper.validJobCardList[0].ETT_Cementing_End_Time_new__c);
...
var datetime = "Last Sync: " + currentdate.getDate() + "/"
                ...
                var $date1 = new Date(respWrapper.relatedJobWrapper.validJobCardList[0].ETT_Cementing_End_Time_new__c);
                var $date2 = currentdate;
                var $interval = $date2 - $date1;
                
                var diff = $interval / 1000;
                diff /= 60;
                console.log(Math.abs(Math.round(diff))+' mints:');
               // return Math.abs(Math.round(diff));
                
                var d = new Date();
                var n = d.getMonth();
                console.log('current month is '+n);
                if(n>=4 && n<=10 && Math.abs(Math.round(diff))<=20){
                    component.set('v.wait20mints',true);
                    component.set('v.wait30mints',false);
                    console.log('wait for 20 mints');
                }else if(n<=4 && n>=10 && Math.abs(Math.round(diff))<=30){
                    component.set('v.wait20mints',false);
                    component.set('v.wait30mints',true);
                    console.log('wait for 30 mints');                    
                }else{
                    component.set('v.wait20mints',false);
                    component.set('v.wait30mints',false);
                ...
                console.log('****respWrapper.holdLineItem.jobLineItem.Id:'+respWrapper.holdLineItem.jobLineItem.Id);
