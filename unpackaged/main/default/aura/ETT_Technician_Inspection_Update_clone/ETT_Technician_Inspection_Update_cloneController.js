({
    doInit: function(component, event, helper) {
        ...
        var act = component.get("c.getInspectionCardTyreLife");
        act.setParams({ recordId : component.get("v.inspectionRecordId") });
        act.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                console.log(result[0]['ETT_Tyre_Life__c']);
                component.set('v.tyreLife',result[0]['ETT_Tyre_Life__c']);
                if(result[0]['ETT_Tyre_Life__c']=='1'){
        ...
        action.setParams({ recordId : component.get("v.inspectionRecordId") });
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                console.log(JSON.stringify(response.getReturnValue()));
                var result = response.getReturnValue();
                //component.set("v.TyreRejectionLineItem",result);   
                
                for(var i=0;i<result.length;i++){
                    if(result[i].Name=='CrownArea'){
                        if('ETT_Number_of_cuts_0_10_mm__c' in result[i]){
                            if(!('Cuts (0 to 10mm)' in dualListValues)){
                                let wrp={
                                    label:'Cuts (0 to 10mm)',
                                    value:'Cuts (0 to 10mm)',
                                    selectedValue: result[i].ETT_Number_of_cuts_0_10_mm__c
                                };
                                selectedCutsNTCList1.push(wrp);
                                dualListValues.push('Cuts (0 to 10mm)'); 
                            }
                        }
                        if('ETT_Number_of_cuts_11_to_25_mm__c' in result[i]){
                            if(!('Cuts (11 to 25mm)' in dualListValues)){
                                let wrp={
                                    label:'Cuts (11 to 25mm)',
                                    value:'Cuts (11 to 25mm)',
                                    selectedValue: result[i].ETT_Number_of_cuts_11_to_25_mm__c
                                };
                                selectedCutsNTCList1.push(wrp);
                                dualListValues.push('Cuts (11 to 25mm)'); 
                            }
                        }
                        if('ETT_Number_of_cuts_26_to_38_mm__c' in result[i]){
                            if(!('Cuts (26 to 38mm)' in dualListValues)){
                                let wrp={
                                    label:'Cuts (26 to 38mm)',
                                    value:'Cuts (26 to 38mm)',
                                    selectedValue: result[i].ETT_Number_of_cuts_26_to_38_mm__c
                                };
                                selectedCutsNTCList1.push(wrp);
                                dualListValues.push('Cuts (26 to 38mm)'); 
                            }
                        }
                        if('ETT_Number_of_cuts_39_to_50_mm__c' in result[i]){
                            if(!('Cuts (39 to 50mm)' in dualListValues)){
                                let wrp={
                                    label:'Cuts (39 to 50mm)',
                                    value:'Cuts (39 to 50mm)',
                                    selectedValue: result[i].ETT_Number_of_cuts_39_to_50_mm__c
                                };
                                selectedCutsNTCList1.push(wrp);
                                dualListValues.push('Cuts (39 to 50mm)'); 
                            }
                        }
                        if('ETT_Number_of_cuts_51_mm_and_above__c' in result[i]){
                            
                            if(!('Cuts (51mm and above)' in dualListValues)){
                                
                                let wrp={
                                    label:'Cuts (51mm and above)',
                                    value:'Cuts (51mm and above)',
                                    selectedValue: result[i].ETT_Number_of_cuts_51_mm_and_above__c
                                };
                                selectedCutsNTCList1.push(wrp);
                                dualListValues.push('Cuts (51mm and above)'); 
                                console.log('hello');
                            }
                        }
                    }
                    component.set("v.selectedCutsNTCList", selectedCutsNTCList1);
                    component.set("v.values", dualListValues);
                    
                    var tyreLife =  component.get('v.tyreLife');
                    
                        //var tyreLife = result[i]['ETT_Inspection_Card__r']['ETT_Tyre_Life__c'];
                        //console.log('tyreLife: '+tyreLife);
                        ...
                        //if(result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c']=='Rejected'){
                            if(tyreLife=='1'){
                                /*if(result[i].Name=='BeadArea'){
                                    ...
                                    if(beadArea!=null && beadArea.length>0){
                                        for(var k=0;k<beadArea.length;k++){
                                            if(beadArea[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                console.log(beadArea[k].label+'  :: '+result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c']);
                                                beadArea[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c']; //'Rejected';
                                            }
                                    ...
                                    helper.handleOnGoodConditionsChange(component, 'BeadArea', 'No');
                                ...
                                if(result[i].Name=='SideWallArea'){
                                    ...
                                    if(SidewallArea!=null && SidewallArea.length>0){
                                        for(var k=0;k<SidewallArea.length;k++){
                                            if(SidewallArea[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                SidewallArea[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }   
                                        }
                                  ...
                                  //  helper.handleOnGoodConditionsChange(component, 'SideWallArea', result[i].ETT_Good_Condition__c);
                                }*/
                                if(result[i].Name=='CrownArea'){
                                    /*
                                    ...
                                    if(CrownArea!=null && CrownArea.length>0){
                                        for(var k=0;k<CrownArea.length;k++){
                                            if(CrownArea[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                CrownArea[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }   
                                        }
                                    ...
                                    console.log('result[i].ETT_Good_Condition__c: '+result[i].ETT_Good_Condition__c);
                                    */
                                    helper.handleOnGoodConditionsChange(component, 'CrownArea', result[i].ETT_Good_Condition__c);
                                }
                                    ...
                                    if(TyreInterior!=null && TyreInterior.length>0){
                                        for(var k=0;k<TyreInterior.length;k++){
                                            if(TyreInterior[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                TyreInterior[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }   
                                        }
                                ...
                                //helper.handleOnGoodConditionsChange(component, 'TyreInterior', result[i].ETT_Good_Condition__c);
                                }
                                    ...
                                    if(AnyArea!=null && AnyArea.length>0){
                                        for(var k=0;k<AnyArea.length;k++){
                                            if(AnyArea[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                AnyArea[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }   
                                        }
                                ...
                                //helper.handleOnGoodConditionsChange(component, 'AnyArea', result[i].ETT_Good_Condition__c);
                                }*/
                            }else{
                                /*if(result[i].Name=='HolesandInjuries'){
                                    ...
                                    if(HolesandInjuries!=null && HolesandInjuries.length>0){
                                        for(var k=0;k<HolesandInjuries.length;k++){
                                            if(HolesandInjuries[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                HolesandInjuries[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }
                                    ...
                                    helper.handleOnGoodConditionsChange(component, 'HolesandInjuries', result[i].ETT_Good_Condition__c);
                                }
                                if(result[i].Name=='MissingLooseTread'){
                                    ...
                                    if(MissingLooseTread!=null && MissingLooseTread.length>0){
                                        for(var k=0;k<MissingLooseTread.length;k++){
                                            if(MissingLooseTread[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                MissingLooseTread[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }
                                    ...
                                    helper.handleOnGoodConditionsChange(component, 'MissingLooseTread', result[i].ETT_Good_Condition__c);
                                }
                                if(result[i].Name=='Cracks'){
                                    ...
                                    if(Cracks!=null && Cracks.length>0){
                                        for(var k=0;k<Cracks.length;k++){
                                            if(Cracks[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                Cracks[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }
                                    ...
                                    helper.handleOnGoodConditionsChange(component, 'Cracks', result[i].ETT_Good_Condition__c);
                                }
                                if(result[i].Name=='BulgesDepressions'){
                                    ...
                                    if(BulgesDepressions!=null && BulgesDepressions.length>0){
                                        for(var k=0;k<BulgesDepressions.length;k++){
                                            if(BulgesDepressions[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                BulgesDepressions[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }
                                    ...
                                    helper.handleOnGoodConditionsChange(component, 'BulgesDepressions', result[i].ETT_Good_Condition__c);
                                }
                                if(result[i].Name=='Miscellaneous'){
                                    ...
                                    if(Miscellaneous!=null && Miscellaneous.length>0){
                                        for(var k=0;k<Miscellaneous.length;k++){
                                            if(Miscellaneous[k].label==result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['Name']){
                                                Miscellaneous[k].value=result[i]['Tyre_Rejection_Sub_Line_Items__r'][j]['ETT_Status__c'];//'Rejected';
                                            }
                                    ...
                                    helper.handleOnGoodConditionsChange(component, 'Miscellaneous', result[i].ETT_Good_Condition__c);
                                }*/
                            }
    ...
    SubmitTyreLifeOne: function(component, event, helper){
		...
		InspectionTechincalSkill.JobCardId = component.get('v.jobCardId');
        InspectionTechincalSkill.jobCardStage = component.get('v.jobCardStage');
        InspectionTechincalSkill.revision = component.get('v.revision');
        ...
        console.log(component.get('v.jobCardId'));
        console.log('jobCardStage: '+component.get('v.jobCardStage'));
        console.log('revision: '+component.get('v.revision'));        
        
        //return false;
        
        var action = component.get("c.tyreInsectionJobCardProcess");
        action.setParams({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var compEvents = component.getEvent("ComplaintMasterEvent");// getting the Instance of event
                compEvents.setParams({ "flag" : true });// setting the attribute of event
        ...
        for(var i=0;i<mainFocusObj.SidewallArea.length;i++){
            ...
            for(var j=0;j<mainFocusObj.SidewallArea[i].inputFields.length;j++){
                if(mainFocusObj.SidewallArea[i].inputFields[j].label.includes('Rejection Name')){
                    ...
                    RejectionName = RejectionName.replace('/',' ');
                }
    ...
    callTyreInterior: function(component, event, helper){
        console.log(JSON.stringify(component.get('v.allCrownAreaOptions')));
        ...
        for(var i=0;i<TyreRejectionLineItem.length;i++){
            if(TyreRejectionLineItem[i].tyreRejectionName == 'CrownArea'){
                TyreRejectionLineItem[i].strRecordTypeName = 'New Tyre and Casing Inspection Procedures';                
                TyreRejectionLineItem[i].lstTyreRejectionSubLineItems = TyreRejectionSubLineItem;
                TyreRejectionLineItem[i].recordId = 'a4E3M000000034vUAA';
