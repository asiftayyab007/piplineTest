({
    ...
    saveRecordHelper : function(component, event, helper) {
        ...
        var utility = component.find("ETST_UtilityMethods");
        var backendMethod = "approveService";
       
        var sr = component.get("v.serviceRequest");
        
         console.log(JSON.stringify(sr))
        var params = {
             "serviceRequestId" : sr.Id,
            "Coordinator_Comments" : sr.ETST_Coordinator_Comments__c
            
        };
        var promise = utility.executeServerCall(component, backendMethod, params);
        promise.then (
            $A.getCallback(function(response) {                
                utility.showToast("School Transport", 'Approved successfully', "success", "dismissible");                
